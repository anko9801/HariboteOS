...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal) {
	...
	}else if (strncmp(cmdline, "wc ", 3) == 0) {
		cmd_wc(cons, fat, cmdline);
	}else if (cmdline[0] != 0) {
...
void cmd_wc(struct CONSOLE *cons, int *fat, char *cmdline) {
	struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
	struct FILEINFO *finfo = file_search(cmdline + 3, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	char *p;
	...
	if (finfo != 0) {
		...
		p = (char *) memman_alloc_4k(memman, finfo->size);
		file_loadfile(finfo->clustno, finfo->size, p, fat, (char *) (ADR_DISKIMG + 0x003e00));
		while (byte < finfo->size) {
			if (p[byte] == 0x0a) line++;
			if (inword == 1 && (p[byte] == 0x0a || p[byte] == 0x09 || p[byte] == ' ')) {
				inword = 0;
			}else if (inword == 0 && !(p[byte] == 0x0a || p[byte] == 0x09 || p[byte] == ' ')) {
				inword = 1;
				word++;
			}
			byte++;
		}
		sprintf(s, "%8d %8d %8d %s\n", line, word, byte, cmdline + 3);
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
	struct MEMMAN *memman = (struct MEMAN *) MEMMAN_ADDR;
